	...
	<view class="wait-lingqu">
		...
		<view class="order-number">						
			...
			<view class="order-number-info">
				<view class="order-number-left">订单编号</view>
			...
			<view class="order-number-info" v-if="active == 0">
		...
		<view class="lingqu-btn" v-if="active == 0">已完成</view>
		<view class="lingqu-btns" v-else @click="handleHexiao">核销完成</view>
	</view>
</template>
<script>
	...
	export default {
		...
		async onLoad(option) {
			...
			if(detail.orde){  // collectRecords 查看订单列表页面	进来的  查看 已完成的订单
				...
				_self.active = 0  // 已完成按钮
				
			}else{			// 扫码  输入订单号进来的  点击核销的订单
